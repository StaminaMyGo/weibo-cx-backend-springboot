	...
	public void addInterceptors(InterceptorRegistry registry) {
		registry.addInterceptor(jwtInterceptor)
				...
				.excludePathPatterns(
						...
						"/api/v1/users/{userId}",        // 根据 ID 查询用户（使用占位符）
						"/api/v1/users/{userId}/followers",     // 粉丝列表
						"/api/v1/users/{userId}/following",     // 关注列表
						// 微博模块 - 公开接口
						"/api/v1/weibos",                // 微博列表
						"/api/v1/weibos/{wbId}",         // 微博详情
						"/api/v1/weibos/{wbId}/comments",     // 评论列表
						// 文件上传 - 公开接口（注册时使用）
